        ...
        <div className="flex gap-2">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger render={<Button size="sm" />}>
              <Plus className="size-3.5 mr-1" /> New Network
            </DialogTrigger>
            <DialogContent>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {list.map((n) => {
                ...
                const isBuiltIn = BUILT_IN.has(n.Name);
                return (
                  <tr key={n.Id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-gray-800">{n.Name}</td>
                    <td className="px-4 py-3"><Badge variant={driverVariant(n.Driver)}>{n.Driver}</Badge></td>
                    <td className="px-4 py-3 text-gray-500 capitalize">{n.Scope}</td>
                    ...
                    <td className="px-4 py-3">
                      <Button
                        size="icon-sm" variant="ghost" className="text-red-500"
                        disabled={busy[n.Id] || isBuiltIn}
                        title={isBuiltIn ? "Built-in networks cannot be removed" : "Remove network"}
